...
// Typage explicite pour éviter (d as any)
...
export default function BooksPage() {
  ...
  useEffect(() => { if (editor) editor.commands.setMark("textStyle", { fontSize: `${fontSize}px` }); }, [fontSize, editor]);

  // ── TOGGLE HELPERS ────────────────────────────────────────────────────────
  const toggleBold    = () => editor?.chain().focus().toggleBold().run();
  const toggleItalic  = () => editor?.chain().focus().toggleItalic().run();
  const toggleJustify = () => {
    const next = !isJustified;
    setIsJustified(next);
    if (next) editor?.chain().focus().setTextAlign("justify").run();
    else      editor?.chain().focus().unsetTextAlign().run();
  };
  const toggleInvisibleChars = () => setShowInvisibleChars(v => !v);

  // ── INSERT BLOCKS (via TipTap insertContent — stable, pas execCommand) ───
  const insertPageBreak = () => {
    editor?.commands.insertContent(
      `<div data-page-break="true" contenteditable="false" class="echo-page-break-block" style="user-select:none;-webkit-user-select:none;border-top:1px dashed rgba(6,182,212,0.35);margin:2rem 0;text-align:center;font-size:9px;color:rgba(6,182,212,0.5);letter-spacing:0.25em;padding-top:6px;cursor:default;">— ${fr?"Saut de page":"Page break"} —</div><p></p>`
    );
  };

  const insertEndInjection = () => {
    editor?.commands.insertContent(
      `<div contenteditable="false" class="echo-injection-block" style="user-select:none;-webkit-user-select:none;margin-top:3rem;padding-top:1rem;border-top:1px solid rgba(6,182,212,0.25);font-size:9px;color:rgba(6,182,212,0.45);text-align:center;letter-spacing:0.2em;cursor:default;">— ${T.inject} —</div><p></p>`
    );
  };
  ...
  const insertTOC = () => {
    ...
    editor.commands.insertContent(
      `<div contenteditable="false" class="echo-toc-block" style="user-select:none;-webkit-user-select:none;border:1px dashed rgba(6,182,212,0.25);padding:14px;border-radius:3px;margin:18px 0;cursor:default;">
  ...
  // ── AUTO-SAVE → Supabase (debounce 2.5s) ─────────────────────────────────
  ...
  const saveToSupabase = useCallback(async () => {
    ...
    // On stocke en tant qu'objet JSON directement (JSONB Supabase)
  ...
  const saveToFile = () => {
    downloadText(JSON.stringify({ bookTitle, chapters }, null, 2), `${bookTitle.replace(/\s+/g,"_")}.echo-book.json`, "application/json");
    setSaveStatus("saved");
  };

  // ── BOOTSTRAP : charger depuis Supabase ──────────────────────────────────
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      if (uid) {
        ...
        if (rows?.[0]) {
          ...
          try {
            // Sécurisation du typage JSONB Supabase : objet ou string selon config colonne
            const rawMessage = rows[0].messages?.[0];
            const payload = typeof rawMessage === "string" ? JSON.parse(rawMessage) : rawMessage;

            if (payload?.bookTitle) setBookTitle(payload.bookTitle);
            if (payload?.chapters?.length) {
              setChapters(payload.chapters);
              setActiveChapter(payload.chapters[0].id);
            }
            setSaveStatus("saved");
            return;
          } catch (err) {
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => { if (resizingRef.current) setEchoPanelWidth(Math.min(480, Math.max(180, window.innerWidth - e.clientX))); };
  ...
  const buildHTML = () => {
    ...
    return `<!DOCTYPE html><html lang="${lang}"><head><meta charset="UTF-8"><title>${bookTitle}</title>
<style>body{font-family:${fontFamily};font-size:${fontSize}px;line-height:${lineHeight};max-width:65ch;margin:3rem auto;padding:2rem;color:#1a1a1a}
...
p{text-align:${isJustified?"justify":"left"};margin-bottom:${paraSpacing}em;${paraIndent?"text-indent:1.5em":""}}
img{max-width:100%;border-radius:4px;margin:.5em 0}
section{break-before:page}@media print{section{page-break-before:always}}</style>
</head><body><h1>${bookTitle}</h1>${body}</body></html>`;
  };
  ...
  const handleExport = (fmt: string) => {
    ...
    if (fmt === "txt") {
      ...
      downloadText(buildHTML(), `${slug}.${fmt === "html" ? "html" : fmt+"_ready.html"}`, "text/html");
    }
  };
  ...
  const sendEcho = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/books`, {
        ...
        body: JSON.stringify({ message: `[Livre: "${bookTitle}" | Extrait: "${excerpt.slice(0,200)}"]\n\n${msg}`, history: echoHistory, source: "books", selectedButtons: [echoMode], userTier: "connected_free", calendarEvents: {} }),
      });
  ...
  const TB = ({ icon, label, active, onClick }: { icon:string; label:string; active?:boolean; onClick:()=>void }) => (
    <button onClick={onClick} title={label}
      className={`group relative w-[42px] h-7 flex items-center justify-center rounded-lg text-[13px] transition-all border select-none ${active?"bg-cyan-500/15 border-cyan-500/40 text-cyan-400":"border-transparent text-zinc-400 hover:bg-zinc-800/60 hover:text-zinc-100 hover:border-zinc-700"}`}>
      {icon}
      <span className="pointer-events-none absolute left-[46px] top-1/2 -translate-y-1/2 bg-zinc-950 text-white text-[9px] px-1.5 py-0.5 rounded-md whitespace-nowrap opacity-0 group-hover:opacity-100 border border-zinc-800 z-50 shadow-lg transition-opacity">{label}</span>
    </button>
  );
      ...
      <div className="fixed inset-0 bg-black flex flex-col z-50">
        ...
        <div className="flex-1 overflow-y-auto flex items-start justify-center px-8 py-12">
          <div className="w-full max-w-[65ch]" style={{fontSize:`${fontSize+2}px`,fontFamily,lineHeight:lineHeight+0.1,color:"#e4e4e7"}}>
            ...
            <div className="text-[11px] uppercase tracking-widest text-cyan-500/60 font-mono mb-4">{chapters.find(c=>c.id===activeChapter)?.title}</div>
  ...
  return (
    <main className="h-screen bg-white dark:bg-black text-black dark:text-white flex overflow-hidden font-sans transition-colors duration-200 selection:bg-cyan-500/30">
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        <div className="w-[100px] shrink-0 border-r border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex flex-col py-2 overflow-y-auto overflow-x-hidden">
          ...
          <div className="px-2 py-1.5 border-b border-zinc-200 dark:border-zinc-800">
            ...
            <div className="grid grid-cols-2 gap-0.5">
              ...
              <TB icon="→" label={T.indent}  onClick={() => editor?.chain().focus().insertContent(" &nbsp; &nbsp; ").run()} />
            </div>
          </div>
          <div className="px-2 py-1.5 border-b border-zinc-200 dark:border-zinc-800">
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-h-0">
          <div className="h-9 shrink-0 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950 flex items-center px-3 gap-2">
            ...
            <button onClick={saveToFile} className="text-[9px] px-2 py-1 rounded border border-zinc-700 text-zinc-400 hover:border-cyan-500/40 hover:text-cyan-400 transition-all font-mono shrink-0">💾 {T.save}</button>
            <div className="relative shrink-0" ref={exportRef}>
              ...
              {showExportMenu && (
                <div className="absolute right-0 mt-1 w-36 rounded-xl bg-zinc-950 border border-zinc-800 shadow-2xl p-1 z-50 flex flex-col gap-0.5">
                  {[{key:"html",label:"HTML"},{key:"docx",label:"DOCX (HTML)"},{key:"pdf",label:"PDF (HTML)"},{key:"epub",label:"EPUB (HTML)"},{key:"txt",label:"TXT"},{key:"json",label:"JSON (.echo-book)"}].map(({key,label}) => (
          ...
          {showSettings && (
            <div className="shrink-0 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/80 dark:bg-zinc-950/80 px-4 py-2 flex flex-wrap gap-x-4 gap-y-2 items-center text-[10px]">
              {[
                ...
                {label:T.justify,      val:isJustified,     set:(v:boolean)=>{setIsJustified(v);if(v)editor?.chain().focus().setTextAlign("justify").run();else editor?.chain().focus().unsetTextAlign().run();}},
          ...
          <div className="flex-1 overflow-hidden min-h-0 relative"
            ...
            {WATER.map((d,i) => (
              <span key={i} aria-hidden="true" className="absolute pointer-events-none select-none z-[1]"
                style={{top:d.top, left:d.left, right:d.right, fontSize:d.sz, transform:`rotate(${d.rot})`, opacity:0.06, filter:"blur(1px)", color:"#06b6d4", lineHeight:1}}>
                {d.g}
              </span>
            ))}
            <div className="absolute inset-0 overflow-y-auto flex items-start justify-center px-4 py-4 z-[2]"
              ...
              <div className={`w-full shadow-2xl border border-zinc-200/10 dark:border-zinc-700/20 rounded-sm relative ${showInvisibleChars?"echo-editor-show-symbols":""}`}
                style={{maxWidth:"860px",minHeight:"calc(100% - 2rem)",paddingTop:showHeader?0:"52px",paddingBottom:showFooter?0:"64px",paddingLeft:mirrorMargins?"72px":"56px",paddingRight:"56px",...pageBgStyle}}>
                ...
                {isEditingTitle ? (
                  <input ref={titleInputRef} value={bookTitle} onChange={e=>setBookTitle(e.target.value)} onBlur={() => setIsEditingTitle(false)} onKeyDown={e=>{if(e.key==="Enter")setIsEditingTitle(false);}} className="w-full text-2xl font-bold bg-transparent border-b border-cyan-500/40 outline-none text-black dark:text-white mb-6 pb-1" autoFocus />
                ) : (
                  <h1 onDoubleClick={() => { setIsEditingTitle(true); setTimeout(()=>titleInputRef.current?.focus(),50); }} title={T.titleHint} className="text-2xl font-bold text-black dark:text-white mb-6 cursor-text pb-1 border-b border-transparent hover:border-cyan-500/10 transition-colors">{bookTitle}</h1>
                )}
                <div className="text-[9px] uppercase tracking-[0.18em] text-zinc-400 dark:text-zinc-500 mb-3 font-mono">{chapters.find(c=>c.id===activeChapter)?.title}</div>
                {view==="edit" ? (
                  <EditorContent editor={editor} className="outline-none min-h-[400px] text-black dark:text-zinc-100 caret-cyan-400 books-editor-tiptap" style={{fontSize:`${fontSize}px`,lineHeight}} />
                ) : (
                  <div className="min-h-[400px] text-black dark:text-zinc-100 books-preview" style={{fontSize:`${fontSize}px`,fontFamily,lineHeight,textAlign:isJustified?"justify":"left"}} dangerouslySetInnerHTML={{__html: currentContent||`<p style="color:rgba(113,113,122,0.35);font-style:italic">${T.noContent}</p>`}} />
                )}
                {showPageNumbers && (
                  <div className="absolute bottom-5 left-0 right-0 text-center text-[9px] text-zinc-400 font-mono pointer-events-none">— {chapters.findIndex(c=>c.id===activeChapter)+1} —</div>
                )}
                {showFooter && (
        ...
        <aside style={isDesktop?{width:echoPanelWidth,flexBasis:echoPanelWidth}:undefined}
          ...
          <div className="h-9 shrink-0 border-b border-zinc-200 dark:border-zinc-800 flex items-center px-3 gap-2">
            ...
            <span className="text-[10px] font-bold font-mono uppercase tracking-wider text-zinc-300">Echo — {fr?"Écriture":"Writing"}</span>
          </div>
            ...
            {echoMessages.length===0 ? (
              ...
              <>
                {echoMessages.map((msg,i) => (
                  <div key={i} className={`text-[10px] leading-relaxed rounded-xl px-2.5 py-1.5 ${msg.role==="user"?"self-end bg-cyan-500/10 border border-cyan-500/20 text-zinc-200 rounded-br-sm max-w-[88%]":"self-start bg-zinc-900 border border-zinc-800 text-zinc-300 rounded-bl-sm max-w-[92%]"}`}>{msg.text}</div>
                ))}
                {echoThinking && <div className="self-start text-[10px] text-zinc-500 bg-zinc-900 border border-zinc-800 rounded-xl rounded-bl-sm px-2.5 py-1.5">...</div>}
              </>
            )}
            <div ref={echoBottomRef} />
          </div>
          <div className="p-2 border-t border-zinc-200 dark:border-zinc-800 flex gap-1.5 shrink-0">
            <textarea value={echoInput} onChange={e=>setEchoInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendEcho();}}} rows={2} placeholder={T.echoInput} className="flex-1 resize-none bg-zinc-900 border border-zinc-800 text-zinc-200 text-[10px] rounded-lg px-2 py-1.5 placeholder-zinc-600 outline-none focus:border-cyan-700/40 leading-relaxed" />
            <button onClick={sendEcho} disabled={echoThinking} className="w-7 self-end bg-cyan-600/15 border border-cyan-500/25 hover:bg-cyan-600/25 disabled:opacity-30 text-cyan-400 rounded-lg text-xs flex items-center justify-center transition-all h-7 shrink-0">➤</button>
          </div>
        </aside>
      </div>
      ...
      <style>{`
        .books-editor-tiptap .ProseMirror { min-height: 400px; outline: none; color: inherit; }
        ...
        .echo-editor-show-symbols .ProseMirror p:after {
          ...
          font-size: 0.95em !important; font-weight: bold !important; font-family: monospace !important;
        }
        .echo-editor-show-symbols .ProseMirror div[data-page-break="true"] {
          outline: 1px dashed rgba(6,182,212,0.4) !important;
          background: rgba(6,182,212,0.02) !important;
        }
        .books-editor-tiptap .ProseMirror h1, .books-preview h1 { font-size:1.6em; font-weight:700; margin-top:1.2em !important; margin-bottom:0.6em !important; border-bottom:1px solid rgba(6,182,212,0.1); padding-bottom:.12em; }
        .books-editor-tiptap .ProseMirror h2, .books-preview h2 { font-size:1.2em; font-weight:600; margin-top:1.6em !important; margin-bottom:0.5em !important; text-transform:uppercase; color:rgb(6,182,212); }
        .books-editor-tiptap .ProseMirror h3, .books-preview h3 { font-size:1.05em; font-weight:600; margin-top:1.2em !important; margin-bottom:0.4em !important; }
