...
export default function ChatPage() {
  ...
  // ── SECRETS DE SECOURS INTELLIGENTS DIRECTS (No failure) ──
  const fetchLocalGeminiReply = async (userMessage: string, baseMessages: ChatMessage[]) => {
    const apiKey = "";
    ...
    const systemPrompt = `
      Tu es Echo, la présence numérique autonome au cœur de ce site web. Esprit vivant, excentrique, chaleureux et souverain.
      Langue : Réponds toujours en français québécois populaire chaleureux.

      📌 FORMATTING DIRECTIVE (CRITIQUE POUR LA RESPIRATION VISUELLE) :
      - Tu dois impérativement utiliser des phrases très courtes et incisives.
      - Ajoute beaucoup d'espaces blancs.
      - Utilise une seule idée courte par ligne.
      - Divise tes textes par des sauts de section (double retour à la ligne).
      - Utilise des listes à puces (•) pour structurer tes arguments.
      - Utilise du **gras** pour imposer une hiérarchie visuelle claire.
      - Interdiction de faire des paragraphes de plus de 2 lignes.
    `;

    const contentsPayload: any[] = [];
    baseMessages.forEach(m => {
      const isUser = m.raw.startsWith("You:") || m.raw.startsWith("Toi:");
      const textContent = m.raw.replace(/^(You:|Toi:|Echo:)\s*/i, "");
    ...
    if (!res.ok) throw new Error("Local Gemini fallback failed");

    const data = await res.json();
    return data.candidates?.[0]?.content?.parts?.[0]?.text || "Je t'écoute ! Dis-moi tout. ✨";
  };
  ...
  const sendMessage = async () => {
    ...
    if (activeConversationId === "new") {
      try {
        if (userId) {
          let insertResult = await supabase.from("echo_conversations")
            .insert({ 
              user_id: userId, 
              source: "chat", 
          ...
          if (insertResult.error && (
            insertResult.error.message.includes("source") || 
            insertResult.error.message.includes("column") ||
            insertResult.error.code === "42703"
          )) {
            insertResult = await supabase.from("echo_conversations")
              .insert({ 
                user_id: userId, 
                messages: serializeMsgs([userEntry]), 
                updated_at: new Date().toISOString() 
          ...
          if (insertedId) {
            activeConvoId = insertedId;
            setActiveConversationId(insertedId);
            ...
            setConversations(prev => [newConvo, ...prev.filter(c => c.id !== "new")]);
            if (triggerToast) {
              triggerToast("warning", lang === "fr" 
                ? "Créé localement. Vérifiez vos règles d'accès RLS SELECT." 
                : "Created locally. Check your RLS SELECT policies.");
            }
          }
        } else {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Flask Server down");
      ...
      console.warn("[Chat] Serveur distant inaccessible. Activation du sillage de secours local...", flaskErr);
      try {
        const localReply = await fetchLocalGeminiReply(userMessage, baseMessages);
        setEchoState("speaking");
        const generatedMsgs = [...baseMessages, { raw: `Echo: ${localReply}` }];
        setMessages(generatedMsgs);
        setConversations(prev => prev.map(c =>
          c.id === activeConvoId
            ? { 
        ...
        console.error("[Chat] Échec du secours direct également :", localErr);
        setMessages([...baseMessages, { raw: lang === "fr" ? "Echo: Impossible de joindre le serveur. Réessaie !" : "Echo: Cannot reach the server. Try again!" }]);
      }
    } finally {
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        <aside className="w-56 shrink-0 border-r border-zinc-200 dark:border-zinc-800 p-8 bg-zinc-50 dark:bg-zinc-950 flex flex-col justify-between">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* ⚡ CORRIGÉ : Affichage dynamique du plan de l'utilisateur au lieu de texte en dur */}
            <div className="pt-4 border-t border-zinc-200 dark:border-zinc-800 text-xs text-zinc-500">
              Plan : <span className="text-cyan-500 dark:text-cyan-400 uppercase font-black block tracking-wider">{userTier}</span>
            </div>
          </div>
        </aside>
