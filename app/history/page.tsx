...
export default function HistoryPage() {
  ...
  const checkAndSaveHistory = (msgs: string[], uid: string | null) => {
    ...
    if (threshold > lastSavedLength.current && threshold > 0) {
      ...
      const currentHistory: HistoryEntry[] = saved ? JSON.parse(saved) : [];
      const updatedHistory = [entry, ...currentHistory];
      setHistory(updatedHistory);
      localStorage.setItem(getHistoryKey(uid), JSON.stringify(updatedHistory));
    }
  };

  // Logique de souris pour manipuler le splitter du milieu
  ...
  const sendMessage = async (forcedText?: string) => {
    ...
    const activeStorageKey = getStorageKey(user.id);
...
const response = await fetch(`${API_URL}/chat`, {
      ...
      if (data.action) {
        ...
        const quotaCategory = (type === "ADD_BUBudget_EXPENSE" || type === "ADD_CALORIE_LOG" || type === "SET_CALORIE_GOAL" || type === "UPDATE_CALORIE_GOAL") ? "vitality" : "calendar";
      ...
      if (data.action) {
        if (data.action.type === "ADD_BUDGET_EXPENSE") {
          ...
          const expensesKey = `echo-budget-expenses-${user.id}`;
          const currentExpenses = JSON.parse(localStorage.getItem(expensesKey) || "[]");
          localStorage.setItem(expensesKey, JSON.stringify([
            ...currentExpenses,
            { id: Date.now().toString(), title: title || "Purchase", amount: parseFloat(amount ?? spent) || 0, date: paymentDate || paidAt || date || new Date().toLocaleDateString("fr-CA") }
          ]));
        }

        if (data.action.type === "ADD_CALORIE_LOG") {
          const { foodName, calories } = data.action.payload;
          const caloriesKey = `echo-calorie-logs-${user.id}`;
          const currentCalories = JSON.parse(localStorage.getItem(caloriesKey) || "[]");
          localStorage.setItem(caloriesKey, JSON.stringify([
            ...currentCalories,
            { id: Date.now().toString(), foodName: foodName || "Food", calories: parseInt(calories) || 0, date: new Date().toLocaleDateString("fr-CA") }
          ]));
        }
        ...
        if (data.action.type === "ADD_CALENDAR_EVENT") {
          ...
          if (dateKey) {
            const existing = JSON.parse(localStorage.getItem(activeStorageKey) || "{}");
            existing[dateKey] = [...(existing[dateKey] || []), newEvent];
            localStorage.setItem(activeStorageKey, JSON.stringify(existing));
          }

          const token = localStorage.getItem(`echo-google-token-${user.id}`);
          if (token && dateKey) {
            const hasTime = start && start.includes(":");
            const startObj = !hasTime ? { date: dateKey } : { dateTime: new Date(`${dateKey}T${start}:00`).toISOString() };
            const endObj = !hasTime ? { date: dateKey } : { dateTime: new Date(`${dateKey}T${end || "23:59"}:00`).toISOString() };

            fetch("https://www.googleapis.com/calendar/v3/calendars/primary/events", {
              method: "POST",
              headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
              body: JSON.stringify({ summary: newEvent.title, description: newEvent.notes || "Added via Echo History", start: startObj, end: endObj }),
            }).catch((err) => console.error("❌ Google transmission pack failed:", err));
          }
        }
      }
    } catch {
      setChatMessages([...baseMessages, "Echo: Unable to reach the server."]);
    }
  };
  ...
  return (
    <main className="h-screen bg-white dark:bg-black text-black dark:text-white flex overflow-hidden relative font-sans transition-colors duration-200">
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        {/* CONTENU SÉCURISÉ OU ÉCRAN DE VERROUILLAGE */}
        {isPageBlocked ? (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <button
                        ...
                        className="text-zinc-400 hover:text-red-500 shrink-0 text-sm"
                      >🗑️</button>
                    </div>
        ...
        {viewingHistory && !isPageBlocked && (
          ...
          >
            <div
              className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-700 rounded-xl w-full max-w-3xl h-[80vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200"
            ...
            >
              <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center bg-zinc-50 dark:bg-zinc-900 shrink-0">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <button
                    onClick={() => handleSendToEcho(viewingHistory)}
                  ...
                  <button
                    onClick={() => setViewingHistory(null)}
                    className="w-8 h-8 flex items-center justify-center rounded-full bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-zinc-700 dark:text-zinc-300 transition text-sm shadow-sm"
                  >
                    ✕
                  </button>
                </div>
                    ...
                    <div key={index} className="whitespace-pre-wrap text-xs">
                      {isEcho ? (
                        <div className="flex items-start gap-3 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3 shadow-sm">
                          ...
                          <div className="flex-1 text-zinc-700 dark:text-zinc-300 leading-relaxed">
                            {msg.replace(/^Echo\s*:\s*/i, "")}
                          </div>
                        </div>
                      ) : isUser ? (
