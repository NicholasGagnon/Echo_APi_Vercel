...
export default function ConversationPage() {
  ...
  const saveConversationToDB = async (uid: string, convId: string | null, raws: string[], summary: string) => {
    ...
    if (raws.length > 10) {
      try {
        const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000";
        const res  = await fetch(`${API_URL}/memory-summary`, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ summary, messages: raws.slice(0, 500), userTier }) });
        const data = await res.json();
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }: any) => {
      ...
      try {
        if (uid) {
          ...
          if ((profile as any)?.user_tier) setUserTier(normalizeTier((profile as any).user_tier));
        } else {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange(async (event: string, session: any) => {
      if (event === "SIGNED_OUT" || !session?.user) {
        setUserId(null); setUserTier("connected_free");
        const empty: Conversation = { id:"new", title: lang==="fr"?"Nouvelle conversation":"New conversation", messages:[], summary:"", updatedAt:Date.now() };
  ...
  const sendMessage = async () => {
    ...
    if (!quotaStatus.allowed) { triggerQuotaPopup(quotaStatus.error === "anon_limit" ? "anon_limit" : (lang==="fr"?"Chat IA":"AI Chat")); return; }
    ...
    if (activeConversationId === "new") {
      ...
      } catch (err) { console.error("[Conv] create:", err); setMessages([...messages, { raw:"Echo: Échec d'initialisation." }]); setEchoState("idle"); return; }
    }
  ...
  return (
    <main className="h-screen bg-white dark:bg-[#0f0f0f] text-zinc-900 dark:text-zinc-100 flex overflow-hidden font-sans selection:bg-cyan-500/20">
      ...
      <aside className={`${sidebarOpen?"w-64":"w-0"} shrink-0 transition-all duration-300 overflow-hidden border-r border-zinc-100 dark:border-zinc-800/60 bg-zinc-50/80 dark:bg-zinc-950/80 flex flex-col`}>
        <div className="p-5 shrink-0">
          <div className="mb-6 px-1">
            <span className="text-sm font-bold text-zinc-700 dark:text-zinc-200">Echo AI</span>
          </div>
            ...
            {[
              ...
              <Link key={item.href} href={item.href}
                className={`block px-3 py-2 rounded-lg text-sm transition-colors ${
                  (item as any).active ? "text-cyan-500 font-semibold" : "text-zinc-500 dark:text-zinc-400 hover:text-cyan-500"
                }`}>
                {item.label}
              </Link>
            ))}
            <div className="my-2 h-px bg-zinc-200 dark:bg-zinc-800 mx-1"/>
            <Link href="/history" className="block px-3 py-2 rounded-lg text-sm text-zinc-500 dark:text-zinc-400 hover:text-cyan-500 transition-colors">
          ...
          <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-600 px-1 mb-2">Conversations</p>
        </div>
        ...
        <div className="p-4 shrink-0 border-t border-zinc-100 dark:border-zinc-800/60">
          <span className="text-[11px] text-zinc-400">Status : <span className="text-cyan-500 font-semibold uppercase">{userTier==="connected_free"?(lang==="fr"?"Accès libre":"FreeConnect"):userTier}</span></span>
        </div>
      </aside>
              ...
              <div className="relative bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-2xl shadow-sm focus-within:border-zinc-400 dark:focus-within:border-zinc-500 transition-colors">
                ...
                <div className="flex items-center justify-between gap-2 px-3 pb-3 pt-1">
                  <div className="flex items-center gap-1">
                    ...
                    <button type="button" onClick={() => isImageLocked ? undefined : imageInputRef.current?.click()}
                      className={`p-1.5 rounded-lg transition-colors ${isImageLocked?"opacity-30 cursor-not-allowed":"text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800"} ${selectedImage?"text-emerald-500":""}`}>
        ...
        <aside className="w-52 shrink-0 border-l border-zinc-100 dark:border-zinc-800/60 flex flex-col bg-zinc-50/50 dark:bg-zinc-950/50 overflow-hidden">
          ...
          <div className="shrink-0 p-4 overflow-y-auto max-h-[55vh]">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <button
                onClick={() => {
                  if (userTier==="connected_free"||userTier==="basic") { triggerQuotaPopup(lang==="fr"?"Émergence (Premium requis)":"Emergence (Premium required)"); return; }
