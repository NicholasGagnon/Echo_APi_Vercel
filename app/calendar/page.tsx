...
export default function CalendarPage() {
  ...
  const [userTier, setUserTier] = useState<"free" | "basic" | "premium" | "ultra" | "founder pack">("free");
  const [showLimitModal, setShowLimitModal] = useState(false);

  // L'accès à la synchronisation manuelle et à l'import/export est désormais ouvert à tous
  const hasSyncAccess = true;
  ...
  const clearStoredGoogleToken = async (uid: string) => {
    ...
    try {
      ...
      console.error("Failed to clear stale Google token", err);
    }
  };

  const reconnectGoogle = async () => {
    const redirectTo =
      typeof window !== "undefined" ? `${window.location.origin}/calendar` : undefined;
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      const { data: profile } = await supabase
        ...
        .eq("id", uid)
        .maybeSingle();
      const tier = (profile?.user_tier || "free") as typeof userTier;
      setUserTier(tier);
      ...
      if (hashToken) {
        ...
        if (row?.google_access_token) {
          ...
          const saved = localStorage.getItem(getGoogleTokenKey(uid));
          if (saved) { activeToken = saved; setGoogleToken(saved); }
        }
      }

      // Tout le monde peut synchroniser à la demande
      if (activeToken) {
        await fetchGoogleEvents(activeToken, uid, tier);
      }
    ...
    const { data: listener } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === "SIGNED_IN" || event === "TOKEN_REFRESHED") {
        ...
        if (providerToken) {
          ...
          fetchGoogleEvents(providerToken, uid, tier);
        } else {
          const { data: row } = await supabase
            .from("user_tokens")
            .select("google_access_token")
  ...
  useEffect(() => {
    ...
    fetchGoogleEvents(googleToken, userId, userTier);
  }, [currentMonth, currentYear, isLoaded, userId, googleToken]);

  const fetchGoogleEvents = async (token: string, uid?: string, tier?: typeof userTier) => {
    if (!token) return;
    ...
    try {
      ...
      if (!res.ok) {
        console.error("Google Calendar sync failed", res.status);
        return;
      }
      ...
      setEvents((prev) => {
        ...
        Object.keys(updated).forEach((dateKey) => {
          if (dateKey.startsWith(monthPrefix)) {
            updated[dateKey] = (updated[dateKey] || []).filter((e) => !e.googleEventId);
          }
        });
        Object.keys(incomingEvents).forEach((dateKey) => {
          const localNonGoogle = (updated[dateKey] || []).filter((e) => !e.googleEventId);
          updated[dateKey] = [...localNonGoogle, ...incomingEvents[dateKey]];
        });
        return updated;
      });
    } catch (err) {
    ...
    reader.onload = () => {
      ...
      lines.forEach((line) => {
        ...
        if (cleanLine === "BEGIN:VEVENT") {
          ...
          const match = cleanLine.match(/(\d{8})T(\d{4})/);
          const matchDate = cleanLine.match(/:(\d{8})$|;VALUE=DATE:(\d{8})/);
          if (match) {
          ...
          const match = cleanLine.match(/(\d{8})T(\d{4})/);
          if (match) cur.end = `${match[2].substring(0, 2)}:${match[2].substring(2, 4)}`;
        } else if (cleanLine.startsWith("SUMMARY:")) {
          cur.title = cleanLine.replace("SUMMARY:", "").trim();
        } else if (cleanLine.startsWith("DESCRIPTION:")) {
          cur.notes = cleanLine.replace("DESCRIPTION:", "").replace(/\\n/g, "\n").trim();
      ...
      try {
        ...
        if (cloudId) {
          setEvents((prev) => {
            ...
            return {
              ...prev,
              [selectedDateKey]: dayEvents.map((e) =>
    ...
    <main className="h-screen bg-white dark:bg-black text-black dark:text-white flex overflow-hidden relative font-sans transition-colors duration-200 selection:bg-cyan-500/30">
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        <section className="flex-1 flex flex-col px-4 sm:px-8 py-12 overflow-y-auto bg-white dark:bg-gradient-to-b dark:from-zinc-950 dark:via-black dark:to-black transition-colors duration-200 min-w-0">
          ...
          {/* CALENDAR HEADER — STRUCTURE EN COMPORTE AVEC LE BON DISTANCEMENT GAUCHE/DROITE */}
          <div className="flex flex-col lg:flex-row lg:items-center justify-start gap-4 mb-8 shrink-0 w-full max-w-7xl mx-auto border-b border-zinc-100 dark:border-zinc-900 pb-5">
            ...
            {/* ACTION TOOLBAR — AJUSTÉE ET TASSEE AVEC LA CLASSE ml-6 POUR FITTER PARFAITEMENT SANS TOUCHER L'EXTRÊME DROITE */}
            <div className="flex items-center gap-2 bg-zinc-50/80 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/80 p-1.5 rounded-xl shadow-sm ml-6 overflow-x-auto max-w-full">
              <button
                onClick={handleManualSync}
                disabled={isSyncing}
                className={`text-[11px] font-bold px-3 py-2 rounded-lg transition-all flex items-center gap-1.5 border ${
                  isSyncing
                    ? "bg-zinc-100 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 text-zinc-400 dark:text-zinc-600 cursor-not-allowed animate-pulse"
              ...
              <button onClick={triggerImportClick} className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 hover:border-cyan-500 text-[11px] font-bold px-3 py-2 rounded-lg transition-colors text-zinc-700 dark:text-zinc-300 shadow-sm shrink-0">
                📥 {lang === "fr" ? "Importer" : "Import"}
              </button>
              <button onClick={triggerExportClick} className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 hover:border-cyan-500 text-[11px] font-bold px-3 py-2 rounded-lg transition-colors text-zinc-700 dark:text-zinc-300 shadow-sm shrink-0">
          ...
          <div className="w-full max-w-7xl mx-auto flex-1 flex flex-col min-h-0 overflow-x-auto">
            <div className="min-w-[600px] flex-1 flex flex-col">
              ...
              <div className="grid grid-cols-7 gap-2 flex-1 auto-rows-fr">
                {blanks.map((_, i) => (
                  <div key={"b" + i} className="bg-zinc-50/20 dark:bg-zinc-950/10 rounded-xl border border-transparent" />
                ))}
                {days.map((day) => {
                  ...
                  return (
                    <button
                      key={day}
                      onClick={() => openDay(day)}
                      className={`min-h-[90px] border rounded-xl p-2 text-left flex flex-col justify-between transition-all group overflow-hidden relative ${
                        mainToday
                          ? "border-cyan-500 bg-cyan-50/5 dark:bg-zinc-900/60 shadow-md shadow-cyan-500/5 filter drop-shadow-[0_0_4px_rgba(6,182,212,0.15)]"
                          : "border-zinc-200 dark:border-zinc-900 bg-zinc-50 dark:bg-zinc-900/20 hover:border-zinc-400 dark:hover:border-zinc-800"
                      }`}
                    >
                      <div className={`font-mono text-xs font-bold ${mainToday ? "text-cyan-500" : "text-zinc-400 dark:text-zinc-500"}`}>
                        {day}
                      </div>
                      <div className="flex-1 w-full space-y-1 mt-1 overflow-hidden flex flex-col justify-start">
                        {dayEvents.slice(0, 2).map((ev) => (
                          ...
                          >
                            <span className="font-bold">{ev.start ? `${ev.start} ` : ""}</span>
                            {ev.title}
                          </div>
                        ))}
                        {dayEvents.length > 2 && (
                          <div className="text-[9px] text-cyan-600 dark:text-cyan-500 font-mono font-bold pl-1">
      ...
      {selectedDateKey && (
        <div
          className="fixed inset-0 bg-black/85 flex items-center justify-center z-50 p-4 backdrop-blur-md animate-in fade-in duration-200"
          onClick={() => setSelectedDateKey(null)}
        >
          <div
            className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 sm:p-8 max-w-xl w-full shadow-2xl space-y-6 max-h-[85vh] overflow-y-auto animate-in zoom-in-95 duration-200"
            ...
            {selectedEvents.length > 0 && (
              <div className="space-y-3">
                {selectedEvents.map((ev) => (
                  <div
                    key={ev.id}
                    className="bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 flex justify-between items-start"
                  >
                    <div className="min-w-0 flex-1 space-y-1">
                      <div className="font-bold text-sm text-zinc-900 dark:text-zinc-100 flex items-center gap-2 flex-wrap">
                        ...
                        {ev.googleEventId && (
                          <span className="text-[9px] bg-zinc-100 dark:bg-zinc-900 px-2 py-0.5 rounded-md border border-zinc-300 dark:border-zinc-700 uppercase tracking-widest font-mono font-bold text-zinc-500 dark:text-zinc-400">Google</span>
                        )}
                      </div>
                    ...
                    <button
                      onClick={() => deleteEvent(selectedDateKey, ev.id, ev.googleEventId)}
                      className="text-zinc-400 hover:text-red-500 font-mono text-sm ml-4 p-1 transition-colors"
                    >
                      🗑️
                    </button>
                  </div>
      ...
      {showLimitModal && (
        <div
          className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-md animate-in fade-in duration-200"
          onClick={() => setShowLimitModal(false)}
        >
          <div
            className="bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 rounded-3xl p-6 sm:p-8 max-w-md w-full shadow-2xl text-center space-y-4 animate-in zoom-in-95 duration-200"
            onClick={(e) => e.stopPropagation()}
          >
