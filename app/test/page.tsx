...
export default function HallPage() {
  ...
  // État pour déclencher l'effet de rotation Pikachu sur le rond central
  const [isPikachuActive, setIsPikachuActive] = useState(false);
  ...
  // Synthétiseur audio "Pika Pika !" via Web Audio API
  const playPikaSound = () => {
    try {
      ...
      const playSyllable = (frequencyStart: number, frequencyEnd: number, startTime: number, duration: number) => {
      ...
      // "Pi-"
      playSyllable(650, 1150, now, 0.12);
      // "-ka"
      playSyllable(1300, 850, now + 0.13, 0.18);
      // "Pi-"
      playSyllable(650, 1150, now + 0.38, 0.12);
      // "-ka !"
      playSyllable(1300, 850, now + 0.51, 0.22);
    } catch (e) {
      console.warn("AudioContext bloqué par le navigateur :", e);
    }
  };

  const handleGlobeClick = () => {
    if (isPikachuActive) return;
    setIsPikachuActive(true);
    playPikaSound();
    
    // Rétablir la rotation classique après 1.2 seconde
    setTimeout(() => {
      setIsPikachuActive(false);
    }, 1200);
  };
  ...
  return (
    <div style={{ background: "#050505", color: "#e8e8e8", minHeight: "100vh", fontFamily: "'Inter', system-ui, sans-serif", overflowX: "hidden" }}>
      ...
      {/* ── HERO — formule d'origine, cercle avec effet de rotation Terre/Pikachu ────────────────────────── */}
      <section style={{ padding: "80px 24px 60px", position: "relative", textAlign: "center" }}>
        ...
        {/* CONTENEUR DU ROND CENTRAL SANS AUCUNE MODIFICATION DE TAILLE */}
        <div 
        ...
        >
          {/* Halo électrique jaune pulsant actif uniquement pendant l'effet Pikachu */}
          <div className={`pikachu-glow-ring ${isPikachuActive ? "active" : ""}`} />
          
          <img 
            src="/affinity.jpg" 
            alt="Affinité de Projets"
            className={`${isPikachuActive ? "animate-pika-spin" : "animate-earth-spin"}`}
            style={{ 
              width: "100%", 
              height: "100%", 
              objectFit: "cover", 
              borderRadius: "50%", 
              opacity: .92, 
              border: `1px solid ${isPikachuActive ? "rgba(255,220,0,0.6)" : "rgba(0,200,255,.25)"}`, 
              boxShadow: isPikachuActive ? "0 0 70px rgba(255,220,0,.35)" : "0 0 60px rgba(0,200,255,.18)",
      ...
      <section style={{ padding: "20px 24px 70px", maxWidth: 800, margin: "0 auto" }}>
        ...
        <Link href="/form" style={{ textDecoration: "none" }}>
          <div style={{
            ...
            display: "flex", alignItems: "center", justifyItems: "center", justifyContent: "space-between", gap: 20, flexWrap: "wrap",
      ...
      {/* ── TROUVEZ LES BONNES PERSONNES — 2 cartes claires ─────────── */}
      ...
      <section style={{ padding: "20px 24px 70px", maxWidth: 800, margin: "0 auto" }}>
        ...
        <Link href="/idea" style={{ textDecoration: "none" }}>
          <div style={{
      ...
      {/* ── STYLES ET ANIMATIONS SÉCURISÉES ── */}
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Rotation continue et majestueuse de la Terre */
        .animate-earth-spin {
        ...
        /* Rotation survoltée Pikachu au clic */
        .animate-pika-spin {
          animation: pikaRotation 0.3s linear infinite;
        }
        @keyframes pikaRotation {
          from { transform: rotate(0deg); }
          to { transform: rotate(360deg); }
        }

        /* Halo électrique jaune autour de l'avatar pendant l'activation */
        .pikachu-glow-ring {
          position: absolute;
          inset: -4px;
          border-radius: 50%;
          border: 2px dashed rgba(255, 220, 0, 0);
          pointer-events: none;
          z-index: 5;
          opacity: 0;
          transition: opacity 0.2s ease-in-out;
        }
        .pikachu-glow-ring.active {
          opacity: 1;
          border-color: rgba(255, 220, 0, 0.7);
          animation: glowSpin 0.5s linear infinite, glowPulse 1s ease-in-out infinite alternate;
        }
        @keyframes glowSpin {
          100% { transform: rotate(360deg); }
        }
        @keyframes glowPulse {
          0% { box-shadow: 0 0 15px rgba(255, 220, 0, 0.2); }
          100% { box-shadow: 0 0 35px rgba(255, 220, 0, 0.6); }
        }
      `}</style>
    </div>
  );
